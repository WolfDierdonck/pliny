...
class PageNameWikiMediaAPI(PageNameDataSource):
    def __init__(self) -> None:
        self.iterator = MediaWikiPageListIterator(
            "!"
        )  # ! is the first lexigraphical page name
...
class PageNameDumpFile(PageNameDataSource):
    ...
    def get_next_page_name(self) -> str:
        while True:
            ...
            if (
                split_line[0] == "en.wikipedia"
                and not any(
                    split_line[1].startswith(start)
                    for start in self.skipped_start_names
                )
