...
class PageRevisionDumpFile(PageRevisionDataSource):
    ...
    def load_dump_file(self, date: Date) -> dict[tuple[str, Date], PageRevisionMetadata]:
        ...
        with open(filename, "r") as f:  
            for line in f:
                ...
                page_date_to_entries[(page, date)].append({
                    "user_id": cols[self.col_name_to_index["user_id"]],
                    "revision_text_bytes": cols[self.col_name_to_index["revision_text_bytes"]],
        ...
        for (page, date), entries in page_date_to_entries.items():
            out[(page.replace("_", " "), date)] = PageRevisionMetadata(
                ...
                editor_count=len(set(entry["user_id"] for entry in entries)),
                net_bytes_change=sum(int(entry["revision_text_bytes"]) for entry in entries),
                revert_count=sum("revert" in entry["revision_tags"] for entry in entries),
            )
